...
const Nav = () => {
  ...
  useEffect(() => {
    ...
    const fetchAccountData = async provider => {
      ...
      let balance = parseFloat(web3.utils.fromWei(await web3.eth.getBalance(selectedAccount)));
  ...
  return <NavBar network={state.network} account={state.account} balance={state.balance} />;
};
